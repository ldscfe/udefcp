...
//
// history: 修改定义方式：const static char S_Box[8][4][16] = {
*/
# include <memory.h>


void DESEncrypt(char *desc, char *src);
void DESDecrypt(char *desc, char *src);


enum {ENCRYPT,DECRYPT};
char key[]={1,5,2,0,3,9,1,6,1,4,0,2,7,5,8};
...
void DESEncrypt(char *desc, char *src)
{
	DESCrypt(desc, src, strlen(src), key, sizeof(key), ENCRYPT);
}
void DESDecrypt(char *desc, char *src)
{
	DESCrypt(desc, src, strlen(src), key, sizeof(key), DECRYPT);
}
